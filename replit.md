...
## Hero Slider Background Opacity Control - New Feature (December 31, 2025)

Added full opacity/transparency control for hero slider background colors - completely customizable color overlay:

**Features Added:**
1. **New Database Column**: `backgroundColorOpacity` (integer 0-100) added to hero_slides table
2. **Admin UI Control**: Added opacity slider in Hero Slides Management admin panel with real-time percentage display
3. **Dynamic Rendering**: Landing page now respects opacity setting when rendering gradient overlay on slider images
4. **Backwards Compatible**: Existing slides default to 100% opacity
5. **Full Transparency Support**: Setting opacity to 0% completely removes the color layer

**How It Works:**
- Users can now set opacity from 0% (fully transparent, see only image) to 100% (solid color)
- When editing a hero slide in the admin panel, there's now a slider control to adjust the color opacity
- The color gradient overlay adjusts its transparency based on the selected opacity percentage
- When opacity is 0%, no color overlay is shown at all - the image is completely visible
- Perfect for showing images without color overlay or with custom transparency levels

**Files Modified:**
- `shared/schema.ts`: Added `backgroundColorOpacity` field to heroSlides table (0-100)
- `client/src/pages/admin/HeroSlidesManagement.tsx`: Added opacity slider in the form with live percentage display
- `client/src/pages/Landing.tsx`: Updated color rendering to use opacity value with special handling for 0% (no color overlay)

## Admin Login Fix & Authentication Correction (December 31, 2025)

Fixed authentication issues and corrected admin user credentials:

**Issues Resolved:**
1. **Admin Password Hash Corrected**: Updated the password hash in the database to match "admin123" correctly using bcrypt
2. **AuthModal Props Removed**: Fixed TypeScript error by removing unsupported props (`mode`, `onModeChange`) from AuthModal component in Landing.tsx that were preventing the login modal from rendering properly
3. **Verified Database Connection**: Confirmed PostgreSQL connection and user data integrity
...
**What Was Fixed:**
- Previous password hash in database didn't match the "admin123" password
- AuthModal component was receiving props it doesn't accept, causing rendering issues
- Both issues have been resolved and the login system is now functional

## Hero Slider Animation Fix - Smooth Transitions (December 31, 2025)

Fixed the flashing/white screen issue when slider transitions between hero slides.

**Changes Made:**
- Changed AnimatePresence mode from "wait" to "sync" in client/src/pages/Landing.tsx
- Simplified transition animation from fade + slide to fade only
- Reduced transition duration from 0.5s to 0.4s for smoother effect
- Result: Seamless slide transitions without momentary blank screens

**Technical Details:**
- Previous: `mode="wait"` caused the exiting slide to fully disappear before the next one entered
- New: `mode="sync"` ensures both animations occur simultaneously for a clean cross-fade effect
